...
var config_user = {
  grid: {
    ...
    header: header,
    url: {
      get    : api_url + "api/pelanggan/all",
      remove : api_url + "api/pelanggan/remove",
      save   : api_url + "api/pelanggan/save",
    },
    method: "GET",
    reorderRows: false,
    autoLoad: true,
    ...
    columns: [
      ...
      },
      {
        ...
        searchable: false,
        render(record, extra) {
          var html = '<a style="padding:2px" href="javascript:remove(\''+record.id+'\',\''+record.nama_pelanggan+'\')" class="btn btn-xs btn-danger">Hapus</a>';
...
function add() {
  ...
  $("#data_title").text("Tambah pelanggan")
  $("#insert").val(1)
...
function edit(data) {
  ...
  $("#data_title").text("Edit pelanggan")
  $("#insert").val(0)
...
$("#post").on('submit', (function(e) {
  e.preventDefault();
  $.ajax({
      url: api_url + 'api/pelanggan/insert',
      type: 'POST',
...
function remove(id,pelanggan)
{
  w2confirm('Anda ingin menghapus data ini? <br> '+pelanggan)
  .yes(() => { 
    $.post(api_url + 'api/pelanggan/remove',
    {
      id:id
    },function(data){
      toastr.success(data.message)
