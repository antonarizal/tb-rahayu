          ...
          onClick: function (event) {
            if (event.target == "pilih") {
              ...
              if (sel.length == 1) {
                ...
                w2alert("Silahkan pilih data yang akan ditambahkan");
              }
            }
            if (event.target == "ganti_harga") {
              var grid = w2ui[grid_r];
              var sel = grid.getSelection();
              if (sel.length == 1) {
                var data = grid.get(sel[0]);
                var selected = w2ui[grid_r].get(w2ui[grid_r].getSelection());
                var row = selected[0];
                gantiHarga(row.harga,row.id)

              }else{
                w2alert("Silahkan pilih data yang akan diedit");
...
function gantiHarga(harga,id) {
  w2prompt({
    ...
    .ok((value) => {
      ...
      $.post(
        server_url + "api/product/jasa/changeprice/"+id,
        {
          harga: value,
