...
function getNota( fakturObj){
    ...
    const hAlamat = limitText(store.get("alamat"),lebar)
    const headAlamat =  hAlamat.split("\n")

    
    
    data += setText(store.get("namaToko"),lebar,"c") + "\n";
    // data += setText(store.get("alamat"),lebar,"c") + "\n";
    $.each( headAlamat , function( id, txt ) {
    ...
    $.each( getFaktur.listBarang , function( idx, val ) {
      ...
      if(val.jenis == "barang"){
        ...
        data += setText(numeral(val.harga).format("0,0"),20,"r");
