function returPenjualan(id) {
    ...
    layout_mode: {
      ...
      panels: [
        ...
        },
        {
          ...
          Faktur Penjualan <input style="height:30px;width:150px;" readonly id="faktur_penjualan" class="w2ui-input">
          ...
          <input style="height:30px;width:150px;" type="hidden" id="pelanggan_id" class="w2ui-input">
    ...
    grid_left: {
      ...
      onLoad: function (event) {
        ...
        var interval = setInterval(function () {
          ...
          var today = new Date();
          var fakturTrx = "RJ." + form_ymdHis(today);
          ...
          $("#faktur_penjualan").val(faktur.faktur);
          $("#faktur_retur").val(fakturTrx);
          $("#pelanggan_id").val(faktur.pelanggan_id);
          $("#date").val(date_now());
        }, 100);
      },
      ...
      toolbar: {
        items: [
          ...
          },
          {
            ...
            html: function (item) {
              var html =
                '<div ><input placeholder="qty"  style="height:25px;width:60px" min=1  id="qty" type="number" class="w2ui-input"><input style="display:none" type="number" id="qmax"></div>';
              return html;
            },
          },
          {
  ...
  $(function () {
    ...
    w2ui[grid_l].url = server_url + "api/penjualan/detail/" + id;
    w2ui[grid_l].reload();
...
function addCart() {
  if (parseFloat($("#qty").val()) > parseFloat($("#qmax").val())) {
...
function selesai() {
  ...
  var insert = {
    ...
    total_hpp: 0,
    laba_rugi: 0,
    keterangan: $("#faktur_penjualan").val(),
  };
  $.post(
    server_url + "api/retur/penjualan/insert",
    {
      data: dataProduct,
      insert: insert,
    },
