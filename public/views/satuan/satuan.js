...
var load_url = store.get('base_url') + "views/satuan";
var api_url = server_url;
// var module = '<?=$module?>';
var header = "Daftar Satuan";
var rand = "";
var name = "grid_data";
var config_user = {
  grid: {
    ...
    header: header,
    url: {
      get    : api_url + "api/satuan/all",
      remove : api_url + "api/satuan/remove",
      save   : api_url + "api/satuan/save",
    },
    method: "GET",
    reorderRows: false,
    autoLoad: true,
    ...
    columns: [
      ...
      },
      {
        ...
        searchable: false,
        render(record, extra) {
          var html = '<a style="padding:2px" href="javascript:remove(\''+record.id+'\',\''+record.satuan+'\')" class="btn btn-xs btn-danger">Hapus</a>';
    ...
    //   return data;
    // },
    sortData: [{ field: "satuan_id", direction: "desc" }],
...
function add() {
  ...
  // $.get(api_url+'api/satuan/getid',function(resp){
...
function remove(id,satuan)
{
  w2confirm('Anda ingin menghapus data ini? <br> '+satuan)
  .yes(() => { 
    $.post(api_url + 'api/satuan/remove',
    {
      id:id
    },function(data){
      toastr.success(data.message)
