...
var config_barang = {
  grid: {
    ...
    header: header,
    url: {
      get : api_url + "api/hutang/all",
      remove : api_url + "api/hutang/remove",
    },
    method: "GET",
    reorderRows: false,
    autoLoad: true,
    ...
    toolbar: {
      items: [
        ...
        // },
        {
          ...
          html: function (item) {
            var html =
              '<div style="padding: 3px 10px;font-weight:bold;font-size:16px"><span id="hutang"></span></div>';
            return html;
          },
        },
      ],
      ...
      onClick: function (event) {
        ...
        if (event.target == "tampilkan") {
          ...
          loadhutang(tgl_mulai, tgl_selesai);
    ...
    columns: [
      ...
      },
      {
        ...
        size: '150px',
        render(record, extra) {
        var html = '<a style="padding:5px" class="btn btn-xs btn-default" href="javascript:detailPembelian(\''+record.id+'\')"><i class="fa fa-info-circle "></i> Detail</a> <a style="padding:5px" class="btn btn-xs btn-default" href="javascript:openBayar(\''+record.id+'\')"><i class="fa fa-reply"></i> Bayar</a>';
        return html;
        }
    },
    ],
...
function loadhutang(tgl_mulai, tgl_selesai) {
  var grid = w2ui[name];
  // var text = keyword.toLowerCase();
  grid.url =
    api_url +
    "api/hutang/all/?tgl_mulai=" +
    tgl_mulai +
    "&tgl_selesai=" +
    tgl_selesai;
  grid.reload();
