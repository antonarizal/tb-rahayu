...
$("#post").on("submit", function (e) {
  ...
  $.ajax({
    url: server_url + "api/laporan/penjualan/kredit",
    type: "POST",
    ...
    success: function (resp) {
      ...
      $.each(resp.data, function (idx, val) {
        $("#laporan > tbody").append(`<tr><td style="text-align:center">${i}.</td><td>${moment(val.date).format("DD-MM-YYYY")}</td><td>${val.faktur}</td><td>${val.nama_pelanggan}</td><td><span class="float-right">Rp.${numeral(val.grand_total).format("0,0")}</span></td><td><span class="float-right">Rp.${numeral(val.hutang_dibayar).format("0,0")}</span></td><td><span class="float-right">Rp.${numeral(val.hutang_sisa).format("0,0")}</span></td></tr>`);
