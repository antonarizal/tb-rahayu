...
$("#post").on("submit", function (e) {
  ...
  $.ajax({
    url: server_url + "api/laporan/tb_pembelian",
    type: "POST",
    ...
    success: function (resp) {
      ...
      $("#tanggal").html(`Tanggal Cetak: ${resp.date} ${resp.time}`)
      var i = 1;
      $.each(resp.data, function (idx, val) {
        $("#laporan > tbody").append(`
        <tr>
        <td style="text-align:center">${i}.</td>
        <td>${moment(val.rcv_date).format("DD-MM-YYYY") != 'Invalid date' ? moment(val.rcv_date).format("DD-MM-YYYY") : ""}</td>
        <td>${moment(val.due_date).format("DD-MM-YYYY") != 'Invalid date' ? moment(val.due_date).format("DD-MM-YYYY") : ""}</td>
        <td>${moment(val.paid_date).format("DD-MM-YYYY") != 'Invalid date' ? moment(val.paid_date).format("DD-MM-YYYY") : ""}</td>
        <td>${val.faktur}</td>
        <td>${val.suppname}</td>
        <td>${val.tempo}</td>
        <td><span class="float-right">Rp.${numeral(val.priceTotal).format("0,0")}</span></td>
        </tr>`);
