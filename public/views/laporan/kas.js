...
$("#post").on("submit", function (e) {
  ...
  $.ajax({
    url: server_url + "api/laporan/kas",
    type: "POST",
    ...
    success: function (resp) {
      ...
      $("#pemasukan").html("Rp. "+numeral(resp.pemasukan).format("0,0"))
      $("#pengeluaran").html("Rp. "+numeral(resp.pengeluaran).format("0,0"))
      ...
      $.each(resp.data, function (idx, val) {
        ...
        <td><span class="float-right">Rp.${numeral(val.pemasukan).format("0,0")}</span></td>
        <td><span class="float-right">Rp.${numeral(val.pengeluaran).format("0,0")}</span></td>
        </tr>`);
