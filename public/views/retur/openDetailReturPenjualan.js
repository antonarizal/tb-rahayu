function detailReturPenjualan(id) {
  var id = id;
  console.log(id);
  ...
  const config_barang = {
    ...
    //   url: server_url + "api/retur/penjualan/detail/" + id,
      tabIndex: 0,
      show: {
        toolbar: true,
        footer: false,
      ...
      toolbar: {
        items: [
          ...
          },
          {
            ...
            html: function (item) {
              var html =
                '<div style="padding: 3px 10px;font-weight:normal;"><span id="faktur_penjualan"></span></div>';
              return html;
            },
          },
        //   { type: "spacer" },
      ...
      onLoad: function (event) {
        ...
        var interval = setInterval(function () {
          ...
          const total_qty = records
          .map((item) => item.satuan!="JASA" ? numeral(item.qty).value() : 0)
          .reduce((prev, curr) => prev + curr, 0);

          $("#item_terjual").html('Terjual : '+total_qty+' Item');
          $("#penjualan").html('Total : Rp.'+numeral(faktur.grand_total).format('0,0'));
          $("#date").html("Tanggal : " + faktur.date);
          $("#faktur").html("Nota : " + faktur.faktur);
          $("#faktur_penjualan").html("Nota Penjualan: " + faktur.keterangan);
  ...
  $(function () {
    ...
    w2ui[grid_r].url = server_url + "api/retur/penjualan/detail/"+id;
    w2ui[grid_r].reload();
